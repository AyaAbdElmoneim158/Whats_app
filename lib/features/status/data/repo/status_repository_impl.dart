...
import '../models/status_image_model.dart';
...
class StatusRepositoryImpl implements StatusRepository {
  ...
  StatusRepositoryImpl(this.fireStore);

  @override
  Future<void> createStatus(StatusModel status) async {
    final statusCollection = fireStore.collection("status");
  ...
  Stream<List<StatusModel>> getStatuses(StatusModel status) {
    final statusCollection = fireStore.collection("status").where("createdAt",
        isGreaterThan: DateTime.now().subtract(
          const Duration(hours: 24),
        ));
  ...
  Future<void> seenStatusUpdate(
    ...
    try {
      final statusDocRef = fireStore.collection("status").doc(statusId);
    ...
    try {
      if (statusDocRef.exists) {
        ...
        if (createdAt
          ...
          stories.addAll(
              status.stories!.map((e) => StatusImageModel.toJsonStatic(e)));
          // final updatedStories = List<StatusImageModel>.from(existingStatusData['stories'])
